...
@section('content')
    <div class="card  ml-5">
        <div class="card-header">
            <h3 class="card-title">
                Edit Quotation ID:{{ $quotation->id }}
            </h3>
        </div>
        <div class="card-body">
            <form action="{{ route('admin.car-quotations.update', ['id' => $quotation->id]) }}" method="POST">
                @csrf
                @method('PUT')

                <h3 class="mb-5">Edit Quotation #{{ $quotation->id }}</h3>
                ...
                <table class="table table-bordered">
                    ...
                    @foreach($quotation->items as $index => $quoteItem)
                        @php
                            $requestItem = $quoteItem->requestItem;
                        @endphp

                        <tr class="quotation-row">
                            <input type="hidden" name="items[{{ $index }}][quotation_item_id]" value="{{ $quoteItem->id }}">
                            <input type="hidden" class="quantity" value="{{ $requestItem->quantity }}">

                            <td>{{ $requestItem->brand->name_en }}</td>
                            <td>{{ $requestItem->model->name_en }}</td>
                            <td>{{ $requestItem->year?->year ?? '-' }}</td>
                ...
                <div class="row mt-5">
                    <div class="col-md-4">
                        ...
                        <input type="number" step="0.01" class="form-control" id="total_amount"
                               name="total_amount" value="{{ $quotation->total_amount }}" readonly>
                    </div>

                    <div class="col-md-4">
                        <label>Total VAT</label>
                        <input type="number" step="0.01" class="form-control" id="vat_amount"
                               name="vat_amount" value="{{ $quotation->vat_amount }}" readonly>
                    </div>

                    <div class="col-md-4">
                        <label>Total With VAT</label>
                        <input type="number" step="0.01" class="form-control" id="total_with_vat"
                               name="total_with_vat" value="{{ $quotation->total_with_vat }}" readonly>
                    </div>
                </div>
