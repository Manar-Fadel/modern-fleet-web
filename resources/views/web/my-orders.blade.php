                    ...
                    <div class="row g-5">
                        @if(count($orders) == 0)
                            <p class="text-center text-2xs">
                                {{ __('web.You have not orders yet') }}
                                <a href="{{ route('order-now', ['type' => 'car']) }}" class="rts-btn btn-primary radius-big icon mt-5" style="margin: auto;">
                                ...
                                <div class="project-wrapper2">
                                    <div class="image-area text-center">
                                        <a href="{{ route('my-orders.view', ['id' => $order->id]) }}">
                                            @if($order->type == 'heavy_vehicle')
                                                <img src="{{ URL::asset("assets/web/images/heavy-vehicle-vector.png") }}" alt="" style="height: 240px;">
                                            @elseif($order->type == 'attachments')
                                                <img src="{{ URL::asset("assets/web/images/attachments-vector.png") }}" alt="" style="height: 240px;">
                                            @else
                                                <img src="{{ URL::asset("assets/web/images/car-vector.png") }}" alt="" style="height: 240px;">
                                            @endif
                                        </a>
                                        @if($order->status == 'PENDING' && count($order->quotations) == 0 )
                                        <span class="tag">
                                            {{ __('web.will get offers soon') }}
                                        </span>
                                        @elseif($order->status == 'PENDING' && count($order->quotations) > 0 )
                                        <span class="tag">
                                        ...
                                        <a href="{{ route('my-orders.view', ['id' => $order->id]) }}" class="gallery-image">
                                            <img src="{{ URL::asset("assets/web/images/icon/image.svg") }}" alt="">
                                            {{ $order->items->sum(fn($item) => $item->images->count()) }}
                                        </a>
                                    </div>
                                    <div class="content-area">
                                        <h6 class="title ">
                                            @foreach($order->items as $item)
                                                <span  class="col-6 flex">
                                                    {{ $item->brand->name }}/ {{ $item->model->name }}/ {{ $item->year->year }}
                                                </span>
                                                <br>
                                            @endforeach
                                        </h6>
                                        ...
                                        <ul class="feature-area">
                                            <li class="col-6 flex">
                                                <i class="far fa-list-numeric active-icon"></i>
                                                {{ __('web.quantity') }} : {{ $order->items->sum('quantity') }}
                                            </li>
                                            <li class="col-6 flex">
                                                <i class="far fa-calendar active-icon"></i>
                                        ...
                                        <div class="button-area">
                                            @if(count($order->quotations) > 0)
                                            <a href="{{ route('my-orders.view', ['id' => $order->id]) }}" class="active" style="font-weight: 600;font-size: 20px;">
                                                {{ count($order->quotations) }} {{ __('web.offer') }}
                                            </a>
                                            @endif
